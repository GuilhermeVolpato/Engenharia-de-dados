...
df_cardapio = df_cardapio.withColumn("DATA_HORA_GOLD", current_timestamp()).withColumn("NOME_ARQUIVO", lit("cardapio"))
...
df_cardapio.write.format('delta').save(f"/mnt/{storageAccountName}/gold/Cardapio")
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/gold/Cardapio').limit(10).display()

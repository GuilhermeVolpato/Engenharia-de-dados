...
df_mesas = df_mesas.withColumn("DATA_HORA_GOLD", current_timestamp()).withColumn("NOME_ARQUIVO", lit("mesas"))
...
df_mesas.write.format('delta').save(f"/mnt/{storageAccountName}/gold/Mesas")
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/gold/Mesas').limit(10).display()

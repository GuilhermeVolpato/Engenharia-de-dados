...
df_ingredientes = df_ingredientes.withColumn("DATA_HORA_GOLD", current_timestamp()).withColumn("NOME_ARQUIVO", lit("ingredientes"))
...
df_ingredientes.write.format('delta').save(f"/mnt/{storageAccountName}/gold/Ingredientes")
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/gold/Ingredientes').limit(10).display()

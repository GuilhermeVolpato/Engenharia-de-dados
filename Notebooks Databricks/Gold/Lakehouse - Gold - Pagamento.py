...
df_pagamento = df_pagamento.withColumn("DATA_HORA_GOLD", current_timestamp()).withColumn("NOME_ARQUIVO", lit("pagamento"))
...
df_pagamento.write.format('delta').save(f"/mnt/{storageAccountName}/gold/Pagamento")
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/gold/Pagamento').limit(10).display()

...
df_pedido = df_pedido.withColumn("DATA_HORA_GOLD", current_timestamp()).withColumn("NOME_ARQUIVO", lit("pedido"))
...
df_pedido.write.format('delta').save(f"/mnt/{storageAccountName}/gold/Pedido")
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/gold/Pedido').limit(10).display()

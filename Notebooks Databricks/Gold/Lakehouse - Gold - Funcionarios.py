...
df_funcionarios = df_funcionarios.withColumn("DATA_HORA_GOLD", current_timestamp()).withColumn("NOME_ARQUIVO", lit("funcionarios"))
...
df_funcionarios.write.format('delta').save(f"/mnt/{storageAccountName}/gold/Funcionarios")
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/gold/Funcionarios').limit(10).display()

...
df_pagamento = spark.read.option("infeschema", "true").option("header", "true").csv(f"/mnt/{storageAccountName}/landing-zone/Pagamento/FilePagamento.csv")
...
display(dbutils.fs.ls(f"/mnt/{storageAccountName}/bronze/Pagamento"))
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/bronze/Pagamento').limit(10).display()

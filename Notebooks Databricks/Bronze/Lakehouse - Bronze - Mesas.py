...
df_mesas = spark.read.option("infeschema", "true").option("header", "true").csv(f"/mnt/{storageAccountName}/landing-zone/Mesas/FileMesas.csv")
...
display(dbutils.fs.ls(f"/mnt/{storageAccountName}/bronze/Mesas"))
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/bronze/Mesas').limit(10).display()

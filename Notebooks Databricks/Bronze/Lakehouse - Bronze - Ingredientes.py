...
df_ingredientes = spark.read.option("infeschema", "true").option("header", "true").csv(f"/mnt/{storageAccountName}/landing-zone/Ingredientes/FileIngredientes.csv")
...
display(dbutils.fs.ls(f"/mnt/{storageAccountName}/bronze/Ingredientes"))
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/bronze/Ingredientes').limit(10).display()

...
df_pedido = spark.read.option("infeschema", "true").option("header", "true").csv(f"/mnt/{storageAccountName}/landing-zone/Pedido/FilePedido.csv")
...
display(dbutils.fs.ls(f"/mnt/{storageAccountName}/bronze/Pedido"))
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/bronze/Pedido').limit(10).display()

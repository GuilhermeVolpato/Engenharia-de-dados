...
df_estoque = spark.read.option("infeschema", "true").option("header", "true").csv(f"/mnt/{storageAccountName}/landing-zone/Estoque/FileEstoque.csv")
...
display(dbutils.fs.ls(f"/mnt/{storageAccountName}/bronze/Estoque"))
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/bronze/Estoque').limit(10).display()

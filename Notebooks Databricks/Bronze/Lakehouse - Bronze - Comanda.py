...
df_comanda = spark.read.option("infeschema", "true").option("header", "true").csv(f"/mnt/{storageAccountName}/landing-zone/Comanda/FileComanda.csv")
...
display(dbutils.fs.ls(f"/mnt/{storageAccountName}/bronze/Comanda"))
...
spark.read.format('delta').load(f'/mnt/{storageAccountName}/bronze/Comanda').limit(10).display()
